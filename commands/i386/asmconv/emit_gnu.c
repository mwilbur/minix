 ...
 */
{
	...
	case '(':
		...
		break;
	case 'B':
		gnu_printf("%%%s", isglob && IS_(e->name) ? e->name + 1 : e->name);
		break;
	case '1':
	case '2':
	case '4':
	case '8':
		gnu_printf("%%%s", isglob && IS_(e->name) ? e->name + 1 : e->name, e->operator);
		break;
	case '+':
	case '-':
	case '~':
	...
	case 'W':
		if (isregister(e->name)) {
			...
			if (deref && a->optype >= BYTE) gnu_putchar('$');
			if (isglob && IS_(e->name))
				gnu_printf("%s", e->name + 1);
			else
				gnu_printf("%s", e->name);
		}
		break;
...
/* Output one instruction and its operands. */
{
	...
	if (a->opcode == DOT_LABEL) {
		...
		if (syms_is_global(a->args->name) && IS_(a->args->name))
			gnu_printf("%s:", a->args->name + 1);
		else
	...
	if (a->opcode == DOT_ALIGN) {
		gnu_printf(".balign\t%s", a->args->name);
	} else
...
/* the areg is implicit to this instruction! */
#if 0
