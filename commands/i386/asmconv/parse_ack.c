...
static expression_t *ack_get_oplist(asm86_t * a, int *pn, int deref)
 ...
 */
{
	...
	if ((e= ack_get_operand(a, pn, deref)) == nil) return nil;

	if ((t= get_token(*pn))->symbol == ',' || t->symbol == ':') {
		o1= e;
		(*pn)++;
		if ((o2= ack_get_oplist(a, pn, deref)) == nil) {
			del_expr(o1);
			return nil;
		}
		e= new_expr();
...
/* Get a pseudo op or machine instruction with arguments. */
{
	...
	if (get_token(1)->type != T_COMMENT && get_token(1)->symbol != ';'
			&& (a->args= ack_get_oplist(a, &n, deref)) == nil) {
		del_asm86(a);
		return nil;
	}
	tn = get_token(n);
	...
	case DOT_ALIGN:
		...
		if (a->args == nil || a->args->operator != 'W'
			...
			parse_warn(1, t,
			  ".align is restricted to one numeric argument. Unpreprocessed file?\n");
		}
		break;
	case DOT_DEFINE:
	case DOT_EXTERN:
...
asm86_t *ack_get_instruction(void)
{
	...
	if (t->type == T_WORD && get_token(1)->symbol == '=') {
		...
		if ((e= ack_get_C_expression(&n, a)) == nil) {
			zap();
			a= ack_get_instruction();
		} else
