...
PUBLIC void proc_dump_regs(struct proc *proc)
{
	struct stackframe_s * regs = &proc->p_reg;

	printf("\nREGS :\n\t");
	printf("%%eax 0x%08x ", regs->retreg);
	printf("%%ebx 0x%08x ", regs->bx);
	printf("%%ecx 0x%08x ", regs->cx);
	printf("%%edx 0x%08x\n\t", regs->dx);
	printf("%%esi 0x%08x ", regs->si);
	printf("%%edi 0x%08x ", regs->di);
	printf("%%esp 0x%08x ", regs->st);
	printf("%%ebp 0x%08x\n", regs->fp);
	printf("Segments :\n\t");
	printf("%%gs 0x%08x ", regs->gs);
	printf("%%fs 0x%08x ", regs->fs);
	printf("%%es 0x%08x\n\t", regs->es);
	printf("%%ds 0x%08x ", regs->ds);
	printf("%%cs 0x%08x ", regs->cs);
	printf("%%ss 0x%08x ", regs->ss);
}
